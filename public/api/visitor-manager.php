...
switch ($action) {
    case 'create_visitor':
        ...
        if (empty($username) || $duration <= 0) {
            json_response(false, 'Le nom d\'utilisateur et une durée valide sont requis.');
        }
        ...
        try {
            ...
            if ($stmt->fetchColumn() > 0) {
                json_response(false, 'Ce nom d\'utilisateur visiteur existe déjà.');
            }
            ...
            json_response(true, 'Visiteur créé avec succès !', [
                'username' => $username,
                'password' => $password,
            ...
            json_response(false, 'Erreur lors de la création du visiteur : ' . $e->getMessage());
        }
        break;
    ...
    case 'get_visitors':
        try {
            ...
            json_response(true, '', $visitors);
        } catch (Exception $e) {
            json_response(false, 'Erreur lors de la récupération des visiteurs : ' . $e->getMessage());
        }
        break;
