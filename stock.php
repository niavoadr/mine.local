...
<body style="background-color: #F5F5DC;"> <style>
        ...
        .table-marron tr:hover { background-color: rgba(92, 64, 51, 0.1); } /* Survol léger */
        ...
        .badge-communication { background-color: #D2B48C; color: var(--marron-fonce); } /* Couleur claire pour la lisibilité */
    ...
    <div class="container-fluid py-4">
        ...
        <div class="row mb-4">
            <div class="col-md-6">
                <div class="card shadow-sm" style="background-color: var(--blanc-creme); border: 1px solid var(--marron-fonce);">
                    <div class="card-header bg-marron-moyen">
                        <h5 class="mb-0"><i class="fas fa-plus"></i> Ajouter un appareil</h5>
                    </div>
                    <div class="card-body">
                        <form id="addDeviceForm">
                            <div class="mb-3">
                                <label for="mac_address" class="form-label" style="color: var(--marron-fonce);">Adresse MAC</label>
                                ...
                                <div class="form-text" style="color: var(--marron-clair);">Format: aa:bb:cc:dd:ee:ff ou aa-bb-cc-dd-ee-ff</div>
                            </div>
                            <div class="mb-3">
                                <label for="department" class="form-label" style="color: var(--marron-fonce);">Département</label>
                            ...
                            <button type="submit" class="btn btn-marron">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover mb-0 table-marron">
                                ...
                                <tbody id="devicesTable">
                                    <tr>
                                        <td colspan="5" class="text-center py-4" style="color: var(--marron-moyen);">
                                            <div class="spinner-border" style="color: var(--marron-moyen);" role="status"></div>
    ...
    <script>
        ...
        function loadDevices() {
            $.post('radius_devices.php', {action: 'get_devices'}, function(response) {
                if (response.success) {
                    ...
                    $('#devicesTable').html('<tr><td colspan="5" class="text-center text-danger" style="color: var(--marron-fonce);">Erreur: ' + response.error + '</td></tr>');
                }
            }, 'json').fail(function() {
                $('#devicesTable').html('<tr><td colspan="5" class="text-center text-danger" style="color: var(--marron-fonce);">Erreur de communication avec le serveur</td></tr>');
            });
        }
        ...
        function displayDevices(devices) {
            let html = '';
            if (devices.length === 0) {
                html = '<tr><td colspan="5" class="text-center" style="color: var(--marron-fonce); font-weight: bold;">Aucun appareil configuré</td></tr>';
            } else {
                devices.forEach(function(device) {
                    ...
                    html += `
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-danger btn-sm" onclick="deleteDevice('${device.mac_address}')" style="background-color: var(--marron-moyen); color: white; border-color: var(--marron-fonce);">
...
<script>
    ...
    function formatDuration(seconds) {
     ...
     <!-- Bouton de déconnexion avec confirmation JavaScript -->
...
// Constante pour le rôle à rediriger (assurez-vous que ce libellé correspond exactement à votre BDD)
/*define('CHEF_DEPT_LABEL', "Administrateur Département");
  ...
  if ($stmt === false || $stmt->bind_param("s", $_POST['username']) === false || $stmt->execute() === false) {
      ...
      if ($result->num_rows > 0) {
        ...
        if (password_verify($_POST['pass'], $row['mot_de_passe_hash'])) {
          ...
          $target_page = "./dashboard_admin.php"; // Redirection par défaut (sans restriction)
          
            // Si le rôle EST  l'Administrateur de Département (ID 5), on affiche le formulaire
                        if ($current_user_role_id == "ADMIN") { 
          
              $target_page = "./dashboard_user.php"; // Redirection pour administrateur avec restriction
          }
